import QRious from "qrious";
import jsPDF from "jspdf";
import {saveAs} from 'file-saver';

export class PrivateKeysGenerator {
  static generate(exportResponse?: any, username?: any, recoveryKey?: any) {
    const document = new jsPDF('p', 'pt', [794, 1123]);
    const addDetails = (doc: any, wallet: any, username: any, recoveryKey: any) => {
      const SmartCard = {
        localization: {
          pdf: {
            smartcash: "SmartCash",
            hereIsYour: "Here is your web Wallet detail.",
            cardDetails: "Web wallet detail",
            beCareful: "Please, be careful and store this information in a secure location.",
            backQRCode: "MASTER SECURITY CODE",
            recoveryKeyNotPuted: "We were unable to identify your Master Security Code",
            name: "USERNAME",
            address: "PUBLIC ADDRESS",
            addressInfo1: "Use it to deposits.",
            privateKey: "PRIVATE KEY",
            privateKeyInfo1: 'Do not share this information.',
            privateKeyInfo2: 'Can be used to access your funds.'
          }
        }
      };
      const qrAddress = new QRious({
        background: "#ffffff",
        foreground: "black",
        level: "L",
        size: "120",
        value: wallet.address
      });

      const qrPrivateKey = new QRious({
        background: "#ffffff",
        foreground: "black",
        level: "L",
        size: "120",
        value: wallet.privateKey
      });

      const qrEncryptedKey = new QRious({
        background: "#ffffff",
        foreground: "black",
        level: "L",
        size: "120",
        value: recoveryKey
      });

      doc.setLineWidth(1);
      doc.addImage("data:image/png;base64,", "PNG", 50, 70, 50, 56.58);
      doc.setTextColor("#000000");
      doc.setFontStyle("bold");
      doc.setFontSize(24);
      doc.text(SmartCard.localization.pdf.smartcash, 110, 100);

      doc.setFontStyle("normal");
      doc.setFontSize(16);
      doc.text(SmartCard.localization.pdf.hereIsYour, 110, 120);

      doc.setFontStyle("bold");
      doc.setFontSize(32);
      doc.text(SmartCard.localization.pdf.cardDetails, 50, 200);

      doc.setFontStyle("normal");
      doc.setFontSize(24);
      doc.text(SmartCard.localization.pdf.beCareful, 50, 230);

      doc.setFontStyle("bold");
      doc.setFontSize(16);
      doc.text(SmartCard.localization.pdf.name, 50, 300);

      doc.line(50, 360, 700, 360);

      doc.setFontStyle("normal");
      doc.text(username, 50, 340);


      doc.setFontStyle("bold");
      doc.text(SmartCard.localization.pdf.backQRCode, 50, 380);
      
      

      if (recoveryKey) {
        doc.setFontStyle("bold");
        const encryptedKeySplit = doc.splitTextToSize(
          recoveryKey,
          400
        );
        let encryptedKeyYaxis = 430;
        encryptedKeySplit.forEach((element: any) => {
          doc.text(element, 50, encryptedKeyYaxis);
          encryptedKeyYaxis += 20;
        });
        doc.addImage(
          qrEncryptedKey.toDataURL("image/png"),
          "PNG",
          550,
          370,
          120,
          120
        );
      } else {
        doc.text(SmartCard.localization.pdf.recoveryKeyNotPuted, 50, 430);
      }

      doc.line(50, 500, 700, 500);

      doc.setFontStyle("bold");
      doc.text(SmartCard.localization.pdf.address, 50, 520);
      doc.text(SmartCard.localization.pdf.privateKey, 350, 520);
      doc.line(340, 500, 340, 800);
      doc.setFontStyle("normal");
      doc.setFontSize(13);
      doc.text(SmartCard.localization.pdf.addressInfo1, 50, 540);
      doc.setFontSize(16);
      doc.setFontStyle("bold");
      doc.setFontSize(13);
      doc.text(SmartCard.localization.pdf.privateKeyInfo1, 350, 540);
      doc.setFontStyle("normal");
      doc.text(SmartCard.localization.pdf.privateKeyInfo2, 350, 560);

      doc.addImage(qrAddress.toDataURL("image/png"), "PNG", 120, 590, 120, 120);
      doc.addImage(
        qrPrivateKey.toDataURL("image/png"),
        "PNG",
        400,
        590,
        120,
        120
      );

      doc.setFontStyle("normal");
      doc.text(wallet.address, 60, 740);
      doc.text(wallet.privateKey, 350, 740);
    };
    
    exportResponse.data.forEach((wallet: any, index: number) => {
      addDetails(document, wallet, username, recoveryKey);
      if (exportResponse.data.length -1 !== index) {
        document.addPage('p', 'pt', [794, 1123]);
      }
    });

    const obj = { filename: `privateKeys_${username}.pdf`, data: document.output('blob') };
    saveAs(obj.data, obj.filename);
  }
}
